...
coco_zip_codes_spatial <- zctas() %>%
  clean_names() %>%
  filter(zcta5ce10 %in% coconino_zip_codes$zipcode)
...
ggplot() +
  ...
  geom_sf(data = coco_zip_codes_spatial, fill = "#8CBDB8") +
  geom_sf(data = coco_county_roads_spatial, alpha = 2/3, size = 1, color = "#545234")
...
# save coconino county boundary spatial data to pin board 
gis_coco_county_spatial <- coco_county_spatial # rename for pin board 

# save to pinboard 
suicide_data %>%
  pin_write(
    x = gis_coco_county_spatial,
    type = "rds",
...
# save coconino county boundary spatial data to pin board 
gis_coco_county_roads_spatial <- coco_county_roads_spatial # rename for pin board 

# save to pinboard 
suicide_data %>%
  pin_write(
    x = gis_coco_county_roads_spatial,
    type = "rds",
...
# save coconino county boundary spatial data to pin board 
gis_coco_zip_codes_spatial <- coco_zip_codes_spatial # rename for pin board 

# save to pinboard 
suicide_data %>%
  pin_write(
    x = gis_coco_zip_codes_spatial,
    type = "rds",
...
# save coconino county boundary spatial data to pin board 
gis_rate_zipcode_spatial <- rate_zipcode_spatial # rename for pin board 

# save to pinboard 
suicide_data %>%
  pin_write(
    x = gis_rate_zipcode_spatial,
    type = "rds",
    title = "Simple feature of rate by zip code within Coconino County",
    description = "Simple feature of rate by zip code within Coconino County. Zip codes are included only for analysis time range, Coconino County resident, and n > 6. Geodetic CRS: NAD83"
  )
